...
def extract_features_single(domain: str) -> np.ndarray:
    ...
    feats = np.array([
        ...
        n_labels ,
        digit_count ,
        digit_count / full_len ,
        alpha_count ,
        alpha_count / full_len ,
        hyphen_count ,
        hyphen_count / full_len ,
        vowel_count ,
        vowel_count / full_len ,
        consonant_count ,
        consonant_count / full_len ,
        unique_chars ,
        unique_chars / full_len ,
        hex_count / full_len ,
        _entropy ( full ) ,
        _ngram_entropy ( full , 2 ) ,
        _ngram_entropy ( full , 3 ) ,
        _longest_run ( full , CONSONANTS ) ,
        _longest_run ( full , set ( "0123456789" ) ) ,
        _consecutive_consonant_ratio ( full ) ,
        _consecutive_digit_ratio ( full ) ,
        _repeated_char_ratio ( full ) ,
        _gini_index ( full ) ,
        int ( digit_count > 0 ) ,
        int ( full [ 0 ] . isdigit ( ) ) if full else 0 ,
        transitions ,
        max ( label_lengths ) ,
        np . mean ( label_lengths ) ,
        np . std ( label_lengths ) ,
        int ( suffix in _COMMON_TLDS ) ,
        sum ( c . isdigit ( ) for c in sld ) / sld_len ,
        _entropy ( sld ) ,
        len ( sub_parts ) ,
        _markov_log_likelihood ( full ) ,
        _kl_div_from_benign ( full ) ,
        _compression_ratio ( full ) ,
    ], dtype=np.float32)
