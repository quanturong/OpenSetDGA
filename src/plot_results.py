...
try:
    ...
    _edge_colors = _face_colors
    _alphas = [0.55 if src == "tranco_tail" else (0.75 if src == "crt.sh" else 0.85)
               for src in _src_stats["src_short"]]

    bars = []
    for i, (src, val) in enumerate(_src_stats[["src_short", "energy"]].itertuples(index=False)):
        b = ax.barh(src, val, color=_face_colors[i], edgecolor=_edge_colors[i],
                    linewidth=1.5, alpha=_alphas[i], zorder=3, height=0.5)
        bars.append(b[0])

    ax.axvline(_ref_med, color=GREEN, linewidth=2, linestyle="--")
    ax.text(_ref_med + 0.15, 0.5, f"known (ID) median ({_ref_med:.2f})",
            transform=ax.get_xaxis_transform(),
            color=GREEN, fontsize=8, va="center", ha="left", rotation=90)
    ...
    ax . set_xlim ( left = ax . get_xlim ( ) [ 0 ] , right = 0.5 )
    ...
    for _lbl, (_sc, _c) in _groups.items():
        ...
        _kde_fn = _gkde(_s, bw_method=0.25)
        _y = _kde_fn(_x_kde)
        ax2.plot(_x_kde, _y, color=_c, linewidth=2, label=_lbl)
        ax2.fill_between(_x_kde, _y, alpha=0.25, color=_c)
